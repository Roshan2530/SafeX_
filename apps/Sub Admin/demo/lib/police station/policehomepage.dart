import 'dart:async';
import 'package:demo/police%20station/policelist.dart';
...
class _policehomepageState extends State<policehomepage> {
  ...
  Future<void> insert(String name, String address, String email, String password, String number, String lat, String log, String link) async {
    ...
    try {
      await apiService.signup_police_station(name, address, email, password, number, lat, log, link);
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Police Station Added successfully!")),
      );
    _clearForm();
    } catch (e) {
      debugPrint("Error adding Police Station: $e");
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Error adding Police Station: $e")),
      );
    } finally {
      setState(() {
        isLoading = false;
  ...
  Future<void> signup() async {
    if (_formKey.currentState!.validate()) {
      await insert(
        _policeStationNameController.text,
    ...
    return Row(
      children: [
        ...
        CircleAvatar(
          ...
          radius: 30,
          child: Icon(Icons.local_police, color: Colors.blue[900], size: 20),
        ),
        Text(
          'Police Station',
          style: TextStyle(
          ...
          Expanded(
            child: ListView(
              ...
              children: [
                ...
                ListTile(
                  leading: Icon(Icons.brightness_6, color: Colors.blue[900]),
      ...
      child: Center(
        child: Form(
          ...
          child: Column(
            ...
            children: [
              Text('Enter Details for Adding Your Police Station',
                  style: TextStyle(color: Colors.grey, fontSize: 16, fontWeight: FontWeight.bold)),
              SizedBox(height: 20),
              _buildTextField(_policeStationNameController, 'Police Station Name', 'Enter Police Station name', Icons.fire_truck),
  ...
  void _clearForm() {
    _policeStationNameController.clear();
