...
// Create this page for updates

class PoliceStationHistory extends StatefulWidget {
  final String email;

  const PoliceStationHistory({Key? key, required this.email}) : super(key: key);

  @override
  _PoliceStationHistoryState createState() => _PoliceStationHistoryState();
}

class _PoliceStationHistoryState extends State<PoliceStationHistory> {
  bool isLoading = true;
  List<Map<String, dynamic>> policeRecords = [];
  String errorMessage = '';

  @override
  void initState() {
    super.initState();
    fetchPoliceStationHistory();
  }

  Future<void> fetchPoliceStationHistory() async {
    try {
      final data = await ApiService().fetchHistoryPoliceStation(widget.email);
      if (data != null && data is List) {
        setState(() {
          policeRecords = List<Map<String, dynamic>>.from(data);
        });
      } else {
        setState(() {
  ...
  Widget _buildRecordList() {
    ...
    final validRecords = policeRecords.where((record) => record.values.any((value) => value != null && value.toString().isNotEmpty)).toList();
