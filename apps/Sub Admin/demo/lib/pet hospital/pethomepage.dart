...
import 'package:demo/pet%20hospital/pethospitallist.dart';
...
class _pethomepageState extends State<pethomepage> {
  ...
  final _pethospitalNameController = TextEditingController();
  ...
  Future<void> insert(
    ...
    try {
      await apiService.signup_pet_hospital(
          name, address, email, password, number, lat, log, link);

      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Pet Hospital added successfully!")),
      );

      _clearForm(); // ← Clear only after success

    } catch (e) {
      debugPrint("Error adding Pet Hospital: $e");
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Error adding Pet Hospital: $e")),
      );
    }
  }
  ...
  Future<void> signup() async {
    if (_formKey.currentState!.validate()) {
      await insert(
        _pethospitalNameController.text,
      ...
      _clearForm(); // ← Clear the fields after successful insert
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor:
          isDarkMode ? themeData.colorScheme.background : Colors.white,
      key: _scaffoldKey,
      appBar: _buildAppBar(themeData),
      drawer: _drawer(),
      body: _addpethospital(),
    );
  }
            ...
            child: Column(
              ...
              children: [
                CircleAvatar(
                  ...
                  radius: 40,
                  child: Icon(Icons.pets,
                      color:Colors.orange[900], size: 40),
                ),
                SizedBox(height: 10),
                Text(
                  'Pet Hospital App',
          ...
          Expanded(
            child: ListView(
              ...
              children: [
                ...
                ListTile(
                  leading: Icon(Icons.brightness_6, color: Colors.orange[900]),
      ...
      child: Center(
        child: Form(
          ...
          child: Column(
            ...
            children: [
              Text('Enter Details for Adding Your Pet Hospital',
                  style: TextStyle(
                      color: Colors.grey,
                      fontSize: 16,
                      fontWeight: FontWeight.bold)),
              SizedBox(height: 20),
              _buildTextField(_pethospitalNameController, 'Pet Hospital Name',
                  'Enter Pet Hospital name', Icons.pets),
              ...
              _buildTextField(
                  ...
                  isEmail: true),
              _buildTextField(
                  _passwordController, 'Password', 'Enter password', Icons.lock,
                  isPassword: true),
              _buildTextField(
  ...
  void _clearForm() {
    _pethospitalNameController.clear();
