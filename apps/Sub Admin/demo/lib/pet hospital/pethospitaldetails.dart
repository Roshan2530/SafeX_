...
class PetHistory extends StatefulWidget {
  ...
  const PetHistory({Key? key, required this.email}) : super(key: key);

  @override
  _PetHistoryState createState() => _PetHistoryState();
}

class _PetHistoryState extends State<PetHistory> {
  bool isLoading = true;
  List<Map<String, dynamic>> petRecords = [];
  String errorMessage = '';
  ...
  Future<void> fetchPetHistory() async {
    try {
      final data = await ApiService().fetchHistorypet_hospital(widget.email);
      if (data != null && data is List) {
        setState(() {
          petRecords = List<Map<String, dynamic>>.from(data);
        });
      } else {
        setState(() {
  ...
  Widget _buildRecordList() {
    ...
    final validRecords = petRecords.where((record) => record.values.any((value) => value != null && value.toString().isNotEmpty)).toList();
