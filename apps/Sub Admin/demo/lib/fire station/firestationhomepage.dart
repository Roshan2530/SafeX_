import 'dart:async';
import 'package:demo/fire%20station/firestationdetails.dart';
import 'package:demo/fire%20station/firestationlist.dart';
...
class firestationhomepage extends StatefulWidget {
  const firestationhomepage({super.key});

  @override
  State<firestationhomepage> createState() => _firestationhomepageState();
}

class _firestationhomepageState extends State<firestationhomepage> {
  final GlobalKey<ScaffoldState> _scaffoldKey = GlobalKey<ScaffoldState>();
  final _formKey = GlobalKey<FormState>();
  final _firestationNameController = TextEditingController();
  ...
  void initState() {
    super.initState();
    // No need to load theme here, it's done in main.dart
  }

  Future<void> insert(String name, String address, String email,
      String password, String number, String lat, String log,
      String link) async {
    setState(() {
      isLoading = true;
    });
    ...
    try {
      await apiService.signup_fire_station(
          name, address, email, password, number, lat, log, link);

      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Fire Station added successfully!")),
      );

      // Clear all text fields after successful submission
      _firestationNameController.clear();
      ...
      debugPrint("Error adding Fire Station: $e");
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Error adding Fire Station: $e")),
      );
    } finally {
      setState(() {
        isLoading = false;
  ...
  Future<void> signup() async {
    if (_formKey.currentState!.validate()) {
      await insert(
        _firestationNameController.text,
  ...
  void _extractLatLong() {
    ...
    if (match != null) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content:
            Text("Invalid Google Maps link. Please enter a valid link.")),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      drawer: _drawer(),
      body: _addfirestation(),
    );
  }

  Widget _addfirestation() {
      ...
      child: Center(
        child: Form(
          ...
          child: Column(
            ...
            children: [
              Text('Enter Details for Adding Your Fire Station',
                  style: TextStyle(
                      color: Colors.grey,
                      fontSize: 16,
                      fontWeight: FontWeight.bold)),
              SizedBox(height: 20),
              _buildTextField(_firestationNameController, 'Fire Station Name',
                  'Enter Fire Station name', Icons.fire_truck),
              _buildTextField(_addressController, 'Address', 'Enter address',
                  Icons.location_on),
              _buildTextField(
                  _emailController, 'Email', 'Enter email', Icons.email,
                  isEmail: true),
              _buildTextField(_passwordController, 'Password',
                  'Enter password', Icons.lock,
                  isPassword: true),
              _buildTextField(
                  _numberController, 'Number', 'Enter number', Icons.phone,
                  isNumber: true),
              _buildTextField(_latitudeController, 'Latitude',
                  'Extracted from Map Link', Icons.map,
                  isDecimal: true, readOnly: true),
              _buildTextField(_longitudeController, 'Longitude',
                  'Extracted from Map Link', Icons.map,
                  isDecimal: true, readOnly: true),
              _buildTextField(_mapLinkController, 'Map Link',
                  'Enter Google Maps link', Icons.link,
                  isURL: true, isMapLink: true),
              SizedBox(height: 20),
              Center(
    ...
    return Row(
      children: [
        ...
        CircleAvatar(
          ...
          radius: 30,
          child: Icon(Icons.local_fire_department, color: Colors.red[900], size: 20),
        ),
        Text(
          'Fire Station',
          style: TextStyle(
  ...
  Widget _drawer() {
    ...
    final isDarkMode = themeData.brightness == Brightness.dark;
    return Drawer(
      backgroundColor: isDarkMode ? themeData.colorScheme.background : Colors.white,  child: Column(
      children: [
        DrawerHeader(
          child: Column(
            ...
            children: [
              CircleAvatar(
                backgroundColor: Colors.red[100],
                radius: 40,
                child: Icon(Icons.local_fire_department, color: Colors.red[900], size: 40),
              ),
              SizedBox(height: 10),
              Text(
                'Fire Station App',
          ...
          child: ListView(
            ...
            children: [
              _buildDrawerItem(
                ...
                text: 'Fire Station List',
                onTap: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => FireStationsListPage()),
                  );
                },
              ),
              ListTile(
                leading: Icon(Icons.brightness_6, color: Colors.red[900]),
          ...
          actions: [
            ...
            TextButton(
              onPressed: () {
                ...
                // Navigate to login page and remove all previous screens from the stack
